...
파라미터 검증 테스트
====================
Merton ≈ 80.4%, r_tilde ≈ -0.0084, sigma_Y ≈ 0.0784
"""
...
class TestDerivedQuantities:
    ...
    def test_r_tilde(self):
        assert P.r_tilde == pytest.approx(-0.0084, abs=1e-4)

    def test_sigma_Y(self):
        assert P.sigma_Y == pytest.approx(0.0784, abs=1e-3)

    def test_merton_total(self):
        """Merton total ≈ 80.4%. 650%이면 R/r 스왑된 것."""
        total = P.Pi_star.sum()
        assert total == pytest.approx(0.804, abs=0.01)
