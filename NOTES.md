...
## 12.8 Figure 5 equal-CE 재생성 + ε-robustness (2026-08-05, 최종)

**`paper/figures/mc_terminal_y010.png` 를 equal-CE 기준으로 교체.**
이전 버전은 VaR을 명목 α=0.10 으로 그려 복지비용이 다른 두 전략을
비교하고 있었다. 이제 **α = 0.081178 (equal-CE)** 을 쓴다 —
`Y0_VaR = 0.888560`, `k_α = 0.673757`. N=10⁶, seed 20260803.

레이아웃은 세로 2단 (위: CDF 전체, 아래: 좌측꼬리 F_T ≤ 0.9), 축·범례·주석
폰트를 확대했다 (리뷰: 현재 판형에서 판독 곤란).

| 패널 | 내용 |
|---|---|
| 위 | atom 점프 주석 **ES 0.478 / VaR 0.496**, VaR 평평 구간 (0.674, 1) 높이 **α=0.0812** 주석, k=1 수직선 |
| 아래 | Q5 점 **ES 0.788 / Merton 0.714 / VaR 0.634**, bottom-5% mean 수직 파선 **ES 0.714 / Merton 0.646 / VaR 0.574**, ES–VaR 교차점 **F_T = 0.837** 마커 |

검산 로그 (스크립트가 출력):
`sample atom_VaR = 0.4965` (이론 0.4963), `sample P(F_T<1) = 0.0807`
(α=0.0812; N=10⁶ 에서 SE ≈ 2.7e−4 이므로 1.8 SE).

**해석 포인트:** equal-CE로 맞추면 VaR의 k_α 가 0.715 → **0.674** 로 더
내려간다. 같은 복지비용을 내면서 보호를 포기하는 구간이 더 넓어지는
것이고, 그 결과 bottom-5% 가 0.592 → **0.574** 로 더 나빠진다. 명목 α=0.10
비교보다 equal-CE 비교가 ES에 더 유리하게 나오는 이유다.

### ε-robustness (`results/table_eps_robust.csv`)

ε 를 바꿔 equal-CE 를 다시 캘리브레이션한 결과. 오더 제시값과 전부
마지막 표기 자릿수 0.5 ulp 이내 일치.

| ε | α_eqCE | ES CE loss | ES bottom-5% | VaR bottom-5% | 조건부 배율 | bottom-5% 개선 |
|---|---|---|---|---|---|---|
| 0.10 | 0.081178 | 2.275% | 0.7129 | 0.5736 | 3.016 | **+24.28%** |
| 0.12 | 0.141896 | 0.753% | 0.6847 | 0.6153 | 2.197 | +11.28% |
| 0.14 | 0.241508 | 0.103% | 0.6599 | 0.6385 | 1.525 | +3.35% |

**ES의 우위는 제약이 빡빡할수록 커진다.** ε 을 0.10 → 0.14 로 풀면
CE loss 가 2.275% → 0.103% 로 떨어지는 대신 bottom-5% 개선폭이
24.3% → 3.4%, 조건부 배율이 3.02 → 1.52 로 줄어든다. ε=0.14 는
ε_M=0.1526 에 가까워 제약이 거의 안 걸리는 영역이라 두 전략이 수렴한다.
즉 baseline ε=0.10 은 "차이가 나타나는 구간"의 안쪽에 있고, 그 사실
자체가 robustness 근거가 된다.
